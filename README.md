...
To address these issues, _task proxy_ component was developed. Task proxy provides FQDNs for each service and checks operation access with [`check_operation_permission`](https://ytsaurus.tech/docs/en/api/commands#check_operation_permission) API method before proxying user request to job.
...
Use the same namespace where Ytsaurus is installed.

Example of `values.yaml`:
```yaml
# Path to task proxy directory in Cypress, default path for table with task services will be //sys/task_proxies/services
...
Create task proxy user with required ACEs, create k8s secret with user token:
```sh
...
kubectl create secret generic task-proxy-token -n ${NAMESPACE} --from-file token
```
...
Task proxy server discovers such operations and write their services data in table with name `discovery` in directory, specified in `dirPath` parameter. Here is the example of data in table, each row describes single service:
...
In last example we illustrated what you can also use auth cookie to make requests, auth cookie has name `YTCypressCookie` is this case. `9090` is separate gRPC port in your ingress, port value may differ in your case.

## Ingress controller tuning

Here is the example of how to tune your ingress controller for task proxy, specifically its gateway and routes.
...
Here is the example of how to create secret `yt-domain-cert` using private key and certificate chain files:
```sh
